...
function auctioninc_ready_meta_box_callback( $post ) {
	...
	$lot_size = get_post_meta($post->ID, 'auctioninc_lot_size', true);
	$lot_size = is_numeric($lot_size) && $lot_size > 0 ? $lot_size : 1;
	...
	echo '<tr>';
	echo '<th>';
	echo '<label for="auctioninc_lot_size">' . __('Lot Size', 'ready_auctioninc') . '</label>';
	echo '</th>';
	echo '<td>';
	echo '<input type="text" name="auctioninc_lot_size" id="auctioninc_lot_size" value="' . esc_attr($lot_size) . '" placeholder="1">';
	echo '<p class="description">' . __('Number of separate boxes required to ship this item.  When setting lot size > 1, set the weight/dims to the average across the boxes and set Package flag to "Separately".', 'ready_auctioninc') . '</p>';
	echo '</td>';
	echo '</tr>';	
	...
	$ondemand_codes = array(
			...
			'USPPP' => __('USPS Retail Ground', 'ready_auctioninc'),
...
function auctioninc_ready_save_meta_box_data( $post_id ) {
	...
	*/
	...
	$lot_size = sanitize_text_field($_POST['auctioninc_lot_size']);
	update_post_meta($post_id, 'auctioninc_lot_size', $lot_size);
